...
const JobDetailsPanel = ({ job }) => {
  ...
  const handleApply = () => {
    ...
    if (!userProfile) {
      alert("Create your profile before applying.");
      setTimeout(() => {
      ...
      <section className='p-4 py-6 border border-br-primary/60 rounded-lg w-full min-h-full md:h-full justify-center mb-2'>
        ...
        <div className='w-full md:h-[calc(100vh-350px)] lg:h-[calc(100vh-310px)] xl:h-[calc(100vh-300px)] overflow-y-scroll scrollable-container'>
          ...
          <div className='my-5 w-full flex justify-center items-center'>
            <button onClick={handleApply} className={`w-[70%] px-4 py-2 bg-light-bg dark:text-primary text-gray-800 font-semibold rounded-xl text-center hover:bg-btn-bg/60 transition-colors hover:text-white duration-200 cursor-pointer ${isMobile ? 'fixed bottom-2' : ''}`}>
              Apply Now
            </button>
          </div>
        </div>
